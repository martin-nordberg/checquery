...
import type {IStatementQrySvc} from "$shared/services/statements/IStatementQrySvc";
...
export class StatementTeeSvc implements IStatementSvc {
    ...
    constructor(
        private qrySvc: IStatementQrySvc,
        private cmdSvcs: IStatementCmdSvc[]
    ) {
    }
    ...
    async findStatementById(statementId: StmtId): Promise<Statement | null> {
        return this.qrySvc.findStatementById(statementId)
    }

    /** Finds the entire list of statements. */
    async findStatementsAll(): Promise<Statement[]> {
        return this.qrySvc.findStatementsAll()
    }
