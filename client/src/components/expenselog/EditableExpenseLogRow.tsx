...
export type ExpenseLogField = 'vendor' | 'description' | 'entryAccount' | 'entryDebit' | 'entryCredit'

type EditableExpenseLogRowProps = {
    lineItem: ExpenseLogLineItem,
    currentAccountName: string,
    isEditing: boolean,
    editDisabled: boolean,
    focusField?: ExpenseLogField | undefined,
...
const EditableExpenseLogRow = (props: EditableExpenseLogRowProps) => {
    const {expSvc, txnSvc} = useServices()
    ...
    createEffect(async () => {
        if (props.isEditing && !transaction()) {
            const txn = await expSvc.findTransaction(props.lineItem.txnId)
            if (txn) {
                setTransaction(txn)
...
export default EditableExpenseLogRow
