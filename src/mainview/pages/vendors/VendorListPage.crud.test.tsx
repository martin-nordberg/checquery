...
describe("VendorListPage -- rendering real (mocked) vendor data", () => {
	it("renders vendors sorted alphabetically", async () => {
	...
	it("status radios default to Active and filter the vendors", async () => {
		findVendorsAllMock.mockResolvedValue([
	...
	it("shows an empty-state message when there are no vendors", async () => {
		const { findByText } = await renderVendorPage();
		await findByText("No vendors yet. Add one to get started.");
	});
});

describe("VendorListPage -- create vendor flow (header '+')", () => {
	it("restricts Default Account to Expense/Income and refetches on save", async () => {
		...
		expect(params).not.toHaveProperty("ctgId");
