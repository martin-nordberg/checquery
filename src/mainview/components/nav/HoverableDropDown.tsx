...
/**
 * A breadcrumb segment that doubles as a hover-triggered dropdown menu: hovering reveals sibling
 * destinations at that nav level, the current page's own entry is excluded from the list.
 */
const HoverableDropDown = (props: HoverableDropDownProps) => {
	...
	const [getOtherOptions, setOtherOptions] = createSignal(
		excludeSelectedOption(Object.keys(props.options), props.selectedOption),
	);
			...
			<ul class="absolute hidden text-gray-700 text-base pt-1 group-hover/main:block min-w-max max-h-122 overflow-y-auto z-20">
				<For each={getOtherOptions()}>
					{(key) => (
						<li>
							<A
								class="bg-gray-100 hover:bg-gray-200 py-2 px-4 flex items-center gap-2 whitespace-nowrap"
								href={props.options[key] ?? "."}
							>
								{props.iconPaths?.[key] ? <NavIcon path={props.iconPaths[key]} /> : null}
								{key}
							</A>
						</li>
					)}
