import {z} from "zod";
import type {IExpenseLogQrySvc} from "$shared/services/expenselog/IExpenseLogQrySvc";
import type {ExpenseLog, ExpenseLogLineItem} from "$shared/domain/expenselog/ExpenseLog";
...
export class ExpenseLogRepo implements IExpenseLogQrySvc {
    ...
    async findExpenseLog(accountId: AcctId): Promise<ExpenseLog | null> {
        return this.db.transaction(async (txn) => {
            // Get the account info (must be EXPENSE type)
            ...
            const lineItems: ExpenseLogLineItem[] = []
